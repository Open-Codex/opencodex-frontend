...
const ProjectCard = () => {
    return (
        <section className="px-6 py-20 bg-[#0d1117] text-center">
            <h3 className="text-2xl font-semibold mb-2">Proyectos Destacados</h3>
            <p className="text-white/70 mb-10">Descubre proyectos innovadores que buscan colaboradores apasionados</p>
            <div className="grid gap-6 md:grid-cols-3 max-w-6xl mx-auto">
                {[1, 2, 3].map((id) => (
                    <div key={id} className="bg-[#161b22] p-6 rounded-xl border border-white/10 text-left">
                        ...
                        <Button variant="link" className="text-[#A855F7] px-0">Ver Proyecto →</Button>
                    </div>
                ))}
            </div>
            <div className="mt-10">
                <Button className="bg-[#A855F7] hover:bg-[#9333ea]">Ver Todos los Proyectos</Button>
            </div>
        </section>
    )
}
